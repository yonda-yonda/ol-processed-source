...
import { getProfile, type Profile } from "../reader/geotiff/Profile";
import Processor, {
  type CreateProcessorProps,
} from "../reader/geotiff/Processor";
import {
  is4326,
  is3857,
...
import { BaseSource, type Options as BaseOptions } from "./Base";
...
export default class GeoTIFF extends BaseSource {
  ...
  private async setup_(options: GeoTIFFProps): Promise<void> {
    if (options.files) this.profile_ = await getProfile(options.files[0]);
    else if (options.urls) this.profile_ = await getProfile(options.urls[0]);
    if (!this.profile_) {
      this.setState("error");
      return;
    }
    ...
    try {
      createOptions.minSize = tileSize;
