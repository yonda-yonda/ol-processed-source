...
import { rotatePixelExtent } from "../../utils";
...
export async function getProfile(
  ...
  let imageExtent: [number, number, number, number] = [
    Infinity,
    Infinity,
    -Infinity,
    -Infinity,
  ...
  const rotatedCoordinates = rotatePixelExtent([0, 0, width, height], angle);
  const rotatedWidth = Math.round(
    rotatedCoordinates[2] - rotatedCoordinates[0],
  );
  const rotatedHeight = Math.round(
    rotatedCoordinates[3] - rotatedCoordinates[1],
  );
